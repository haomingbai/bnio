// Defer-accept variant of bnio_throughput_benchmark.cpp: the accept loop
// initiates its accepts through the defer scheduler (get_defer_scheduler()),
// while session I/O still uses the post scheduler. The defer scheduler
// bypasses the worker-local queue fast path and publishes every submission
// to the shared queues, so accept re-arms rotate across all workers instead
// of staying pinned to one worker's connection affinity. Everything else
// (arguments, output format, coroutine plumbing) is kept identical to the
// post-scheduler benchmark for A/B fairness comparison.
