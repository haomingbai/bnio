...
function(bnio_resolve_wrk_dependency)
  if(NOT BNIO_FETCH_WRK)
    return()
  endif()
  ...
  # ---- OpenSSL prefix (wrk needs OpenSSL; bnio already requires it) ----
  find_package(OpenSSL REQUIRED)
  get_filename_component(_bnio_openssl_prefix "${OPENSSL_INCLUDE_DIR}" DIRECTORY)
  ...
  if(NOT _LUAJIT_FOUND)
    message(FATAL_ERROR
      "BNIO_FETCH_WRK=ON requires LuaJIT development headers.\n"
      "  Fedora:   sudo dnf install luajit-devel\n"
      "  Debian:   sudo apt install libluajit-dev")
  endif()

  if(_LUAJIT_PREFIX)
    set(_bnio_wrk_luajit_flag "WITH_LUAJIT=${_LUAJIT_PREFIX}")
  else()
    get_filename_component(_lj_prefix "${_LUAJIT_INCLUDE_DIRS}" DIRECTORY)
    set(_bnio_wrk_luajit_flag "WITH_LUAJIT=${_lj_prefix}")
  endif()
  ...
  set(_bnio_wrk_install_dir "${CMAKE_BINARY_DIR}/wrk-install")
  ...
  ExternalProject_Add(wrk_ext
    ...
    BUILD_COMMAND
      $(MAKE) -j
      "WITH_OPENSSL=${_bnio_openssl_prefix}"
      ${_bnio_wrk_luajit_flag}

    BUILD_IN_SOURCE TRUE

    INSTALL_COMMAND
      ${CMAKE_COMMAND} -E make_directory "${_bnio_wrk_install_dir}/bin"
      COMMAND
        ${CMAKE_COMMAND} -E copy_if_different
        <SOURCE_DIR>/wrk
        "${_bnio_wrk_install_dir}/bin/wrk"

    BUILD_BYPRODUCTS "${_bnio_wrk_install_dir}/bin/wrk"
  )
