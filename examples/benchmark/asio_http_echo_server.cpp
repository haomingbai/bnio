...
constexpr std::uint16_t k_default_port = 8081;
...
class session : public std::enable_shared_from_this<session> {
 ...
 private:
  ...
  void on_write(const std::error_code& error, std::size_t bytes) {
    if (error || bytes == 0) {
      return;
    }
...
int main(int argc, char** argv) {
  ...
  signals.async_wait(
      [&context](const std::error_code&, int) { context.stop(); });

  server echo_server(context, port);
  echo_server.start();

  std::cout << "asio_http_echo_server: listening on http://127.0.0.1:" << port
            << '\n';
  context.run();
  std::cout << "asio_http_echo_server: stopped\n";
  return 0;
}
