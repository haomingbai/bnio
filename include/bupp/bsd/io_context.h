...
#define BUPP_BSD_IO_CONTEXT_H_

#include <bupp/async_io/bsd/kqueue_context.h>
#include <bupp/async_io/bsd/kqueue_operations/poll.h>
...
#include <bupp/bsd/detail/steady_timer.h>
...
/**
 * High-level asynchronous I/O context for BSD.
 *
...
class BUPP_EXPORT io_context {
 public:
  ...
  using operation_base = async_io::bsd_native::kqueue_io_operation_base;
  ...
  class schedule_sender {
   public:
    ...
    class operation : public async_io::bsd_native::kqueue_operation_base {
     public:
      /**
  ...
  class basic_scheduler {
   public:
    ...
    /**
     * Returns the native kqueue context used by this scheduler.
     */
    [[nodiscard]] async_io::bsd_native::kqueue_context& native_context()
        const noexcept {
  ...
  /**
   * Copy construction is disabled because the context owns a kqueue context
   * and synchronization resources.
   */
  io_context(const io_context&) = delete;

  /**
   * Copy assignment is disabled because the context owns a kqueue context
   * and synchronization resources.
   */
 ...
 private:
  ...
  /**
   * Returns the underlying BSD native async I/O context.
   */
  [[nodiscard]] async_io::bsd_native::kqueue_context&
  ...
  /**
   * Publishes an operation for passive kqueue submission by a worker.
   */
   ...
   */
  void post(async_io::bsd_native::kqueue_operation_base& operation) noexcept;
  ...
  [[nodiscard]] bool try_fetch_timeout_operations(time_point& deadline,
                                                  bool& fetched) noexcept;
  [[nodiscard]] static bool try_fetch_timeout_operations_thunk(
      void* state, time_point& deadline, bool& fetched) noexcept;
  ...
  async_io::bsd_native::kqueue_task_queue_state global_state_;
...
#include <bupp/bsd/detail/io_context_native_io.h>
#include <bupp/bsd/detail/io_context_state/native_worker.h>
