...
void kqueue_context::finish() noexcept {
  ...
  // custom receivers may start new I/O via publish_io(), which lands
  // in local_state_.io through the worker-local fast path.  Phase 3
  // above only drains the CPU queue, so those new I/O tasks would be
  // stranded permanently.  consume_io_tasks() picks them up, registers
  // them, and abort_inflight_io() cancels them.
  if (consume_io_tasks()) {
    abort_inflight_io();
    for (;;) {
      (void)consume_timeout_operations();
