...
void io_uring_context::finish() noexcept {
  ...
  // callbacks.  When set_stopped() propagates through receiver chains,
  // custom receivers may start new I/O via publish_io(), which pushes
  // to the shared global I/O queue.  Phase 3 above only drains the
  // local task queue, so those new I/O tasks would be stranded.
  // consume_io_tasks() picks them up, prepares SQEs, and
  // abort_inflight_io() cancels anything that was submitted.
  if (consume_io_tasks()) {
    abort_inflight_io();
    for (;;) {
      (void)move_cpu_tasks();
