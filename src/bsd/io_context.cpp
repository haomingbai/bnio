...
io_context::io_context(const io_context_options& options) noexcept
    : native_(options.platform) {
  {
    ...
    // native context. Every actual kqueue is created by the thread that calls
    // run().
    async_io::bsd_native::kqueue_context probe(native_.options.kqueue);
...
detail::native_worker* io_context::register_run_worker() noexcept {
  ...
  auto* worker =
      new (std::nothrow) native_worker(*this, native_.options.kqueue);
