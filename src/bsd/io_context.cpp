...
io_context::io_context(const io_context_options& options) noexcept
    : native_(options.platform, global_state_), timer_driver_operation_(*this) {
...
detail::native_worker& io_context::select_worker() noexcept {
  ...
  async_io::bsd_native::kqueue_context* selected_context =
  ...
  while (current != selected) {
    ...
    async_io::bsd_native::kqueue_context* current_context =
...
async_io::bsd_native::kqueue_context&
...
detail::native_worker* io_context::register_run_worker() noexcept {
  ...
  if (index == 0) {
    ...
    worker->owned_context =
        std::make_unique<async_io::bsd_native::kqueue_context>(
            native_.options.kqueue);
