...
TEST(ProbeTest, behavior) {
  ...
  EXPECT_TRUE(empty_probe.raw() == nullptr);
  EXPECT_TRUE(!empty_probe.is_open());
  EXPECT_TRUE(empty_probe.opcode_supported(IORING_OP_NOP) == 0);

  bupp::base::probe global_probe;
  static_cast<void>(global_probe.get_probe());
  if (global_probe.is_open()) {
    EXPECT_TRUE(global_probe.raw() != nullptr);
    static_cast<void>(global_probe.opcode_supported(IORING_OP_NOP));
  }
  global_probe.free_probe();
  EXPECT_TRUE(!global_probe.is_open());
