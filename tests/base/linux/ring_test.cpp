...
TEST(RingTest, nop_round_trip) {
  ...
  EXPECT_TRUE(ring.is_open());
  EXPECT_TRUE(ring.raw() != nullptr);

  bupp::base::submission_queue_entry sqe = ring.get_sqe();
  EXPECT_TRUE(sqe.raw() != nullptr);
  ...
  EXPECT_TRUE(wait_result == 0);
  EXPECT_TRUE(cqe.raw() != nullptr);
  EXPECT_TRUE(cqe.res() == 0);
  EXPECT_TRUE(cqe.get_data64() == k_user_data);
...
TEST(RingTest, consume_ready_cqes_respects_window) {
  ...
  EXPECT_TRUE(first_batch == 2);
  EXPECT_TRUE(seen_count == 2);

  const unsigned second_batch = ring.consume_ready_cqes(2, record_cqe);
  EXPECT_TRUE(second_batch == 1);
  EXPECT_TRUE(seen_count == k_count);
  EXPECT_TRUE(seen_sum == (k_user_data * k_count) + 0 + 1 + 2);
}
...
TEST(RingTest, disabled_single_issuer_is_claimed_by_enabling_thread) {
  ...
  EXPECT_TRUE(enable_result == 0);
  EXPECT_TRUE(submit_result > 0);
  EXPECT_TRUE(wait_result == 0);
  EXPECT_TRUE(cqe_result == 0);
  EXPECT_TRUE(cqe_data == k_user_data);
}
