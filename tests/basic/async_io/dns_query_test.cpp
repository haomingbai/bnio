...
TEST(DnsTest, query_defaults_and_setters) {
  bnio::async_io::dns_query query("localhost", "http");
  ...
  EXPECT_EQ(query.transport(), bnio::async_io::dns_transport::tcp);
  EXPECT_EQ(query.flags(), bnio::async_io::dns_query_flags::none);
  ...
  query.set_transport(bnio::async_io::dns_transport::udp);
  ...
  bnio::async_io::basic_dns_query<4, 4> small_query("toolong", "80");
...
TEST(DnsTest, query_capacity_edges_and_empty_view) {
  bnio::async_io::basic_dns_query<4, 1> query;
...
TEST(DnsTest, result_view) {
  using endpoint = bnio::async_io::ip::endpoint;

  std::array<endpoint, 2> static_storage{};
  bnio::async_io::dns_result_view static_view(static_storage);
  ...
  bnio::async_io::dns_result_view dynamic_view(dynamic_storage.data(),
...
TEST(DnsTest, dns_category) {
  std::error_code error = bnio::async_io::make_dns_error_code(-2);
  EXPECT_EQ(error.category(), bnio::async_io::dns_category());
  EXPECT_EQ(std::string_view(error.category().name()), "bnio.dns");
...
TEST(DnsTest, behavior) {
  static_assert(std::is_same_v<bnio::async_io::dns_result_view::endpoint_type,
                               bnio::async_io::ip::endpoint>);
}
