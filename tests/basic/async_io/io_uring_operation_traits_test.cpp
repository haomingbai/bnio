...
TEST(IoUringOperationTraitsTest, eagain_rearm_classification) {
  ...
  io_uring_accept_operation accept_operation(context, listener, 0, receiver{});
  EXPECT_TRUE(accept_operation.rearm_on_eagain());
  io_uring_connect_operation connect_operation(context, stream, remote_endpoint,
                                               receiver{});
  ...
  io_uring_send_to_operation send_to_operation(context, datagram, buffer,
                                               remote_endpoint, 0, receiver{});
  EXPECT_TRUE(send_to_operation.rearm_on_eagain());
  io_uring_recvmsg_operation recvmsg_operation(context, stream, receive_message,
                                               0, receiver{});
  ...
  bnio::async_io::duration timeout{};
  io_uring_timeout_operation timeout_operation(context, timeout, 0, 0,
                                               receiver{});
