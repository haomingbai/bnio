...
void test_metadata_helpers(io_uring_sqe& raw_sqe) {
  ...
  EXPECT_EQ(raw_sqe.user_data, k_user_data);
}
...
void test_network_preps(io_uring_sqe& raw_sqe) {
  ...
  EXPECT_EQ(raw_sqe.opcode, IORING_OP_CONNECT);

  sqe = reset_sqe(raw_sqe);
  sqe.prep_send(-1, buffer.data(), buffer.size(), 0);
  EXPECT_EQ(raw_sqe.opcode, IORING_OP_SEND);

  sqe = reset_sqe(raw_sqe);
  sqe.prep_sendmsg(-1, &message, 0);
  EXPECT_EQ(raw_sqe.opcode, IORING_OP_SENDMSG);

  sqe = reset_sqe(raw_sqe);
  sqe.prep_recv(-1, buffer.data(), buffer.size(), 0);
  EXPECT_EQ(raw_sqe.opcode, IORING_OP_RECV);

  sqe = reset_sqe(raw_sqe);
  sqe.prep_recvmsg(-1, &message, 0);
  EXPECT_EQ(raw_sqe.opcode, IORING_OP_RECVMSG);
}
...
void test_poll_and_timeout_preps(io_uring_sqe& raw_sqe) {
  ...
  EXPECT_EQ(raw_sqe.opcode, IORING_OP_TIMEOUT_REMOVE);
}
...
void test_filesystem_preps(io_uring_sqe& raw_sqe) {
  ...
  EXPECT_EQ(raw_sqe.opcode, IORING_OP_WRITE);

  sqe = reset_sqe(raw_sqe);
  sqe.prep_readv(-1, &iov, 1, 0);
  EXPECT_EQ(raw_sqe.opcode, IORING_OP_READV);

  sqe = reset_sqe(raw_sqe);
  sqe.prep_writev(-1, &iov, 1, 0);
  EXPECT_EQ(raw_sqe.opcode, IORING_OP_WRITEV);
}
