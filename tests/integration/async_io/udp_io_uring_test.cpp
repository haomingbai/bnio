...
TEST(UdpIoUringTest, send_to_receive_from_operations) {
  ...
  EXPECT_EQ(state.sent, static_cast<int>(payload.size()));
  EXPECT_EQ(state.received, static_cast<int>(payload.size()));
...
TEST(UdpIoUringTest, connected_datagram_operations) {
  ...
  EXPECT_FALSE(sender_socket.connect(receiver_endpoint));
