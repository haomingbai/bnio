...
TEST(DnsTest, sender_concepts) {
  ...
  std::array<bnio::ip::endpoint, 4> results{};
  bnio::dns_result_view result_view(results);

  using query_sender = decltype(scheduler.async_resolve(
      bnio::dns_query("127.0.0.1", "80"), result_view));
  using string_sender =
  ...
  static_assert(
      bnio::resolves_dns<bnio::io_context::post_scheduler, bnio::dns_query>);

  auto sender =
      scheduler.async_resolve(bnio::dns_query("127.0.0.1", "80"), result_view);
...
TEST(DnsTest, scheduler_resolves_numeric_address) {
  ...
  auto scheduler = context.get_post_scheduler();
  bnio::dns_query query("127.0.0.1", "8080");
  query.set_address_version(bnio::ip::address::version::v4);
  ...
  auto sender = bnio::async_resolve(scheduler, std::move(query),
  ...
  EXPECT_EQ(results[0].address().type(), bnio::ip::address::version::v4);
}
...
TEST(DnsTest, scheduler_resolves_host_service_strings) {
  ...
  auto scheduler = context.get_post_scheduler();
  std::vector<bnio::ip::endpoint> results(8);
  resolve_receiver receiver;
  resolve_state state;
  receiver.state = &state;
  receiver.context = &context;

  auto sender = bnio::async_resolve(
      scheduler, "127.0.0.1", "8081",
      bnio::dns_result_view(results.data(), results.size()));
  auto operation = bexec::connect(std::move(sender), std::move(receiver));
  bexec::start(operation);
  context.run();
