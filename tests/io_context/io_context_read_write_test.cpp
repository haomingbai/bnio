...
TEST(IoContextReadWriteTest, ready_socket_read_completes_without_queue) {
  ...
  int sockets[2] = {-1, -1};
  EXPECT_TRUE(::socketpair(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0, sockets) ==
              0);
  bupp::tcp_socket receiver_socket(sockets[0]);
  bupp::tcp_socket sender_socket(sockets[1]);

  constexpr std::string_view payload = "ready";
  EXPECT_TRUE(::send(sender_socket.native_handle(), payload.data(),
                     payload.size(),
  ...
  EXPECT_TRUE(state->size == payload.size());
...
TEST(IoContextReadWriteTest, ready_socket_write_completes_without_queue) {
  ...
  EXPECT_TRUE(::recv(receiver_socket.native_handle(), bytes.data(),
...
TEST(IoContextReadWriteTest, blocked_socket_write_falls_back_to_queue) {
  ...
  EXPECT_TRUE(::fcntl(sender_socket.native_handle(), F_SETFL,
...
TEST(IoContextReadWriteTest, io_idle_drain_read_write_pair) {
  ...
  EXPECT_TRUE(read_state->size == payload.size());
  EXPECT_TRUE(write_state->signal == signal_kind::value);
  EXPECT_TRUE(write_state->size == payload.size());
  EXPECT_TRUE(std::memcmp(bytes.data(), payload.data(), payload.size()) == 0);
}
...
TEST(IoContextReadWriteTest, file_write_and_read) {
  ...
  EXPECT_TRUE(fd >= 0);
  EXPECT_TRUE(::unlink(path.c_str()) == 0);
...

  {
    ...
    EXPECT_TRUE(::pread(fd, started_bytes.data(), started_bytes.size(), 0) ==
