...
TEST(SslConceptTest, ssl_raii_objects_construct) {
  ...
  EXPECT_TRUE(stream.native_handle() == stream.get_native_handle());
  EXPECT_TRUE(stream.lowest_layer().get_native_handle() == -1);
}

TEST(SslConceptTest, ssl_context_errors_and_ownership) {
  const std::error_code synthetic_error = bupp::make_openssl_error(1);
  EXPECT_TRUE(synthetic_error.value() == 1);
  EXPECT_TRUE(synthetic_error.category() == bupp::openssl_error_category());
  EXPECT_TRUE(std::string_view(synthetic_error.category().name()) == "openssl");
  EXPECT_TRUE(!synthetic_error.message().empty());
  ...
  EXPECT_TRUE(!client_context.valid());
  EXPECT_TRUE(moved_context.native_handle() == client_handle);

  generic_context = std::move(moved_context);
  EXPECT_TRUE(!moved_context.valid());
  EXPECT_TRUE(generic_context.native_handle() == client_handle);
  ...
  EXPECT_TRUE(certificate_error.category() == bupp::openssl_error_category());
  ...
  EXPECT_TRUE(key_error.category() == bupp::openssl_error_category());

  ERR_clear_error();
  const std::error_code mismatch_error = server_context.check_private_key();
  EXPECT_TRUE(mismatch_error);
  EXPECT_TRUE(mismatch_error.category() == bupp::openssl_error_category());
}
