...
TEST(TcpTest, tcp_type_namespace) {
  ...
  EXPECT_TRUE(bupp::ip::tcp::v4().version() == bupp::ip::address::version::v4);
  EXPECT_TRUE(bupp::ip::tcp::v6().version() == bupp::ip::address::version::v6);
  EXPECT_TRUE(bupp::ip::tcp::v4().async_io_protocol().version() ==
              bupp::async_io::ip::address::version::v4);
}

TEST(TcpTest, tcp_owner_aliases) {
  bupp::ip::tcp::socket socket(3);
  EXPECT_TRUE(socket.native_handle() == 3);
  EXPECT_TRUE(socket.release() == 3);

  bupp::ip::tcp::acceptor acceptor(4);
  EXPECT_TRUE(acceptor.native_handle() == 4);
  EXPECT_TRUE(acceptor.release() == 4);
}
...
TEST(TcpTest, socket_owns_and_replaces_descriptors) {
  ...
  EXPECT_TRUE(!socket.open(bupp::ip::tcp::v4()));
  ...
  EXPECT_TRUE(!socket.is_open());
  EXPECT_TRUE(moved.native_handle() == transferred_fd);

  bupp::tcp::socket destination;
  EXPECT_TRUE(!destination.open(bupp::ip::tcp::v4()));
  ...
  EXPECT_TRUE(!replacement.open(bupp::ip::tcp::v4()));
