...
void run_stress_test() {
  ...
  EXPECT_EQ(state.completions.load(std::memory_order_acquire),
            datagram_count * 2);
  ...
  for (std::size_t index = 0; index < datagram_count; ++index) {
    EXPECT_EQ(state.receive_sizes[index], datagram_size);
    EXPECT_EQ(state.send_sizes[index], datagram_size);
    EXPECT_TRUE(sources[index].address().is_v4());
    EXPECT_EQ(sources[index].address().to_v4(),
              sender_endpoint.address().to_v4());
    EXPECT_EQ(sources[index].port(), sender_endpoint.port());

    std::uint32_t value = 0;
    std::memcpy(&value, received[index].data(), sizeof(value));
    EXPECT_LT(value, datagram_count);
